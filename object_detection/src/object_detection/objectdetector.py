    ...
    def __init__(self, config):
        ...
        if self.architecture == "yolo":
            if self.model_dir_path:
                ...
                self.detector = DetectMultiBackend(
                    os.path.join(self.model_dir_path, self.model + ".pt"), device=device
                )
                ...
                self.detector = torch.hub.load(
                    "ultralytics/yolov5", self.model, device=self.device
                )  # 'yolov5n'
            ...
            raise NotImplementedError(
                "Detectron return type was not adapted. \
                                                     See 'detect' function. \
            ...
            self.cfg.merge_from_file(
                model_zoo.get_config_file(self.model)
